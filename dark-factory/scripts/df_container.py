...
#
# M30 (DF-03) hardening: also UNSETS NO_PROXY/no_proxy before exec'ing the
# builder. A NO_PROXY inherited from the base image or a parent build
# context (this entrypoint doesn't set one; it could arrive some other way)
# would make an HTTP(S)_PROXY-respecting client (any libc/language runtime
# that honors both) bypass the proxy entirely for matching hosts -- i.e.
# connect DIRECTLY, straight into the iptables default-deny-egress wall
# this same entrypoint just installed, turning a would-be provider request
# into a silent hang/connection-refused instead of the intended "reach the
# provider only via the injecting proxy" path. Unsetting it removes that
# footgun outright rather than trying to enumerate what it might contain.
...
# M30 (DF-03): df_proxy now requires every forwarded request to present
# Proxy-Authorization: Bearer <capability_token> -- this is the PROBE's own
# (self-test-only) capability token, read from an env var this container was
# started with, never a provider credential. Absent -> "" -> no header sent,
# same as before M30 (the caller didn't opt into the hardened proxy check).
cap_token = os.environ.get("DF_PROBE_CAP_TOKEN", "")
...
try:
    ...
    if cap_token:
        req.add_header("Proxy-Authorization", "Bearer " + cap_token)
...
def probe_enterprise_egress(image, proxy_endpoint, allowed_url, denied_host, *,
                            ...
                            runner=subprocess.run, capability_token=None):
    ...
    capability_token (M30/DF-03): if the proxy at `proxy_endpoint` enforces a
    capability token (df_proxy.serve's `capability_token`/`capability_token_env`
    -- true by default since M30), pass the SAME token here so the in-
    container probe request presents `Proxy-Authorization: Bearer <token>`;
    omitted -> no header is sent (the probe request is refused with
    `allowed_reachable: False` against a token-enforcing proxy, same as any
    other unauthenticated client). This is a probe-only self-test token,
    passed into the container as a plain `-e` var -- never a provider
    credential, and never mounted/echoed anywhere in `detail`.
    """
    ...
    try:
        ...
        probe_env = {"DF_PROBE_CAP_TOKEN": capability_token} if capability_token else None
        try:
