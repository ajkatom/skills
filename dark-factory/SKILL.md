...
- `references/modes.md` — the four intervention modes (H1 directed / H2 supervised / H3 guarded / H4 lights-out), the full state-transition (pause-point) table, the legacy `(autonomy,checkpoint)→mode` mapping + `df-migrate-config`, the H4 fail-closed contract, and the before-ship approval pause (landed in M36b; H1/H2, `AWAIT_SHIP`)
