...
- **Use a distinct key per control root.** The off-box truncation-detection layer
  (see "Off-box sink" and "Honest limits" below) identifies a control root by its
  audit-key fingerprint. Pointing two different control roots at the SAME
  `audit.key_path` aliases their off-box identity slot; the effect is fail-SAFE
  (conservative false-positive truncation refusals, never a missed truncation), but
  it will block otherwise-valid runs. The default path is a single shared file, so
  give each control root its own `audit.key_path` when you run more than one.
...
- **`http-append`**: PUTs the entry to `{audit.sink.url}/audit/{invocation}` against a `df_audit_receiver.py`-compatible receiver. The reference receiver (`python3 df_audit_receiver.py --port N --store-dir DIR`, binding loopback by default — pass `--host 0.0.0.0` only for a genuine off-box deployment, since it has no auth; or `df_audit_receiver.serve(...)` in-process for tests) accepts a PUT to a NEW key (201) and refuses PUT to an existing key (409, no overwrite) — there is no DELETE route at all. That absence of a delete/overwrite code path *is* the append-only guarantee, not merely a documented policy.
...
**Chain TAIL truncation is undetectable from the local chain ALONE — this is the linear-chain limit the off-box sink exists to close.** Each entry links BACKWARD (to `prev_chain_hash`); nothing links forward to a not-yet-written successor. So deleting the most-recent N entries from `audit-chain.jsonl` leaves a shorter chain that is still perfectly internally consistent — `verify-chain` (the bare chain-integrity check) reports `OK` on it, because there is no local record that those trailing entries ever existed (the number of runs isn't independently pinned anywhere on the box). This is exactly what the off-box append-only sink closes: the sink already recorded those entries in a different trust domain, so a third party comparing the box's chain head against the sink's contents sees the missing tail immediately. (Mid-chain deletion/edit, by contrast, IS caught locally — it breaks every forward link from that point.)

**With an off-box sink configured, the supervisor now detects tail-truncation AUTOMATICALLY (M73 → M84), not only by manual third-party comparison.** As each chain entry is written, the supervisor commits a **write-once length checkpoint** to the sink under a key `dfchain.<ns>.<len>` (zero-padded length), where `<ns>` is a namespace derived from the audit-key fingerprint bound to a stable, relocation-invariant **control-root identity** — a write-once off-box slot `dfroot.<fp(key)>` (M83), so moving or renaming the control-root directory does not mint a fresh namespace. Every signed recovery path that trusts the chain — resume (source-identity and resumable-state authentication), ship recovery and re-entry, and spec-fork — calls `_verify_chain_untruncated` first: it probes the sink for the checkpoint one past the local chain's current length, and a **hit means the local chain was truncated** → it refuses fail-closed. (Custody, waiver, and release verification reach the same guarantee by authenticating the sealed manifest's HMAC, M79, rather than re-probing the chain.) Checkpoints are kept **dense** by backfill (the invariant `dfchain.<ns>.L` committed ⇒ `1..L` all committed, M84), so a single dropped write cannot leave a hole a later truncation could hide behind. The production evidence path (`evidence-bundle --require-production`) goes further and requires the completeness state `confirmed_offbox` (M82): a sink-less or unreachable-sink run is runnable but never receives a production-ready verdict.

**Scope and the key/identity requirement.** This automatic detection is *conditional on a reachable off-box sink*: with `audit.sink.required: true` (mandatory at enterprise) it is fail-closed; sink-less or optional-sink standard runs remain detection-grade best-effort, exactly as the linear-chain limit above describes. The chain's backward linkage hash is itself unkeyed, so a same-user writer can excise a middle entry and re-link — but that SHORTENS the chain, which the off-box length checkpoint catches. Because the checkpoint namespace and the `dfroot` identity slot are keyed by the audit-key fingerprint, **each control root must use its own audit key** (see "Signed manifest" above): sharing one key across control roots aliases their identity slot and yields conservative false-positive truncation refusals (fail-safe — never a missed truncation, but it blocks valid runs).
