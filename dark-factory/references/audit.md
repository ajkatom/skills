...
- `prev_chain_hash` is the previous entry's `chain_hash` (or 64 zeros — `GENESIS` — for the control root's first run ever). Because each `chain_hash` folds in the previous one, silently deleting or editing any entry (including the oldest) breaks every link computed from that point forward.
...
**Where it's recorded — sidecars, never manifest fields.** Because the chain entry binds the manifest's *already-finalized* digest, the chain/sink results cannot be written back into that same manifest — embedding them would change the very bytes the entry hashed. So each run_dir gets two sidecars instead: `audit_chain.json` (this run's chain entry, a copy of what landed in `audit-chain.jsonl`) and, only when a sink is configured, `audit_sink_receipt.json`. The manifest itself is written exactly once, by `finalize_manifest`, and is never re-finalized to add these fields. The corresponding journal events (`AUDIT_CHAINED`, and `AUDIT_SINK_OK`/`AUDIT_SINK_WARN`/`AUDIT_SINK_FAILED`) land in `journal.jsonl` AFTER the terminal state — `verify-manifest`'s journal-integrity check accounts for this explicitly (see below); it is not a gap.
...
**`verify-manifest`'s journal check, updated for the chain (M13).** `_anchor_audit` journals `AUDIT_CHAINED` (and `AUDIT_SINK_*`) into `journal.jsonl` immediately AFTER a manifest is finalized — necessarily after, since the chain entry binds that manifest's own digest. `finalize_manifest` now also records `journal_bytes` (the journal's exact size at finalize time) alongside `journal_sha256`, and `verify-manifest` hashes only that PREFIX rather than the whole (now-longer) file — content up to the terminal state must still be byte-identical, but the expected post-finalize audit-chain append doesn't trip a false `TAMPERED`. Anything appended beyond the recorded prefix is still checked, just more narrowly: every trailing line must parse as ndjson whose `state` is one of the known audit-anchor events (`AUDIT_CHAINED`/`AUDIT_SINK_OK`/`AUDIT_SINK_WARN`/`AUDIT_SINK_FAILED`) — a hand-forged or arbitrary trailing line still reads `TAMPERED`. This does not vouch for the trailing entries' CONTENT (only `verify-chain` against `audit-chain.jsonl` does that) — only that nothing arbitrary was spliced into the journal after the terminal state.
